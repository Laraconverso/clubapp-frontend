...
import WritableDropdown from './WritableDropdown';
...
describe('<WritableDropdown />', () => {
  test('Should render component with Colombia and Argentina labels', async () => {
    render(<WritableDropdown options={options} onChange={jest.fn} />);
  ...
  test('Component should return object with selected option when is clicked', async () => {
    ...
    render(
      <div>
        <button></button>
        <WritableDropdown options={options} onChange={onChange} />
      </div>
    );
