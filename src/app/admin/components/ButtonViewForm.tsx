...
const ButtonViewForm = () => {
  ...
  return (
    <div className='flex flex-col md:flex-row items-center justify-between '>
      <div className='bg-baltic-sea-700 grid grid-cols-2  gap-5 m-3 text-center p-10 font-squada  md:gap-20 max-w-sm mx-auto'>
    ...
    <FormOption img={addMatch} title="Alta partido" setForm={setFormToShow} option="createMatch"/>
      ...
      <div className="bg-baltic-sea-300 size-full">
        {(() => {
          switch (formToShow) {
            case "createUser":
...
const FormOption = ({img, title, setForm, option}:{img: StaticImport, title: string, setForm: Dispatch<SetStateAction<FormToShow>>, option: FormToShow}) => {
  ...
  return (
    <section onClick={()=>setForm(option)} className='flex flex-col justify-around bg-baltic-sea-900 px-3 pb-2 rounded-md text-lg border-primary-500 text-primary-50 focus:bg-baltic-sea-950 focus:border-[5px] transition box-border outline-none' tabIndex={0}>
      <p>{title}</p>
